...
- **Simulacro familiar** — a guided 3-minute family drill (Agáchate–Cúbrete–Sujétate) with phase timers, vibration cues, a post-drill family debrief, and a recurring .ics reminder. Practice is what makes the reaction automatic.
- **Read-aloud guides** — every guide can be spoken with the on-device Spanish voice (Web Speech API, works offline on Android) — for low-literacy and low-vision users.
- **Light & dark themes** — dark by default (blackout- and OLED-friendly), adjustable text size up to "Enorme", bold-text and high-contrast modes.
